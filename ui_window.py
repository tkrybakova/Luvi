...
from text_to_speech import TTSUnavailableError, TextToSpeech
    ...
    def _process_input(self, text: str, from_voice: bool = False) -> None:
        ...
        def worker() -> None:
            ...
            try:
                ...
                if not self._tts_warning_shown:
                    self._tts_warning_shown = True
                    self.messages.put(("status", f"TTS disabled: {exc}"))
            except Exception as exc:
                self.messages.put(("status", f"TTS runtime error: {exc}"))
